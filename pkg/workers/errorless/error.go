...
func InfoCloseProcessStdoutReader(worker string, id string) {
	info(worker, fmt.Sprintf("Read from stdout for async task #%s completed, successfully!", id))
}
...
func ErrorProcessKilled(worker string, id string, pid int, err error) {
	warning(worker, fmt.Sprintf(`The async task #%s process PID %d was terminated with an error, the task is removed from the pool and will be restarted in the future. Previous error '%s'`, id, pid, err))
}
